...
export function buildGraph(nodeList: GraphInputNode[], rootId: string): DerivedGraph {
  ...
  // ── Validate references + per-kind shape ──
  for (const n of nodeList) {
    // A node needs *some* way onto the screen: an area, a parent, or — for a
    // relation-anchored (floating) node — at least one relation it can ride along.
    if (
      n.kind === 'node' &&
    ...
    for (const a of n.area ?? []) {
      ...
      if (kindOf(a) !== 'area') throw new Error(`Node "${n.id}" lists non-area "${a}" in area`)
    }
    for (const p of n.parent ?? []) {
      ...
      if (kindOf(p) === 'group') throw new Error(`Node "${n.id}" lists a group "${p}" as a parent`)
    }
    for (const g of n.groups ?? []) {
      if (!byId.has(g)) throw new Error(`Node "${n.id}" references unknown group "${g}"`)
      if (kindOf(g) === 'area') throw new Error(`Node "${n.id}" lists an area "${g}" in groups`)
    }
  ...
  // ── Build edges: parent→node (the tree), group→member, relation. `area` is a
  //    gate-only tag and draws no line. ──
  ...
  // ── Drop label-only groups with a single member: a `kind: 'group'` node that labels
  //    just one node adds no grouping, so hide it and its lone spoke. ──
  ...
  // ── Containment adjacency (group + relation edges excluded) ──
  // The root keeps its area children in `childrenByParent` (so the renderer can detect
  // root mode), but it is not a `branchId` — it stays pinned at the centre, not a
  // clickable category.
  ...
  // appearing whenever a node it relates to is visible — so it is exempt from the
  // reachability check below. (An `area`, if present, just keeps it near that hub.)
  const relationAnchoredIds = new Set(
    nodeList
      .filter((n) => n.kind === 'node' && !n.parent?.length && n.relations?.length)
      .map((n) => n.id)
  )

  // ── Reachability: every non-floating tech node must hang off an area through containment ──
  ...
  // ── Area gate per node: its own `area` tags override; otherwise inherit the union of
  //    its parents' gates. ──
  ...
  // ── Materialize drawn nodes: optional root + areas + groups + tech nodes ──
  // The root exists only when an area attaches to it; it is pinned at the centre.
